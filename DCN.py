...
parser.add_argument("--render", type=int, default=2,
                    help="render level while training, 0: no render, 1: render once after die, 2[default]: render "
                         "every step")
...
parser.add_argument("--visual", type=int, default=2,
                    help="the manhattan distance that snakes can see, note that this argument will affect the model's "
                         "parameter size, if you plan to load a model, pay attention to the corresponding.")
...
class DDQN:
    def __init__(self, input_shape, num_act, env: Snake, gamma=0.99, lamda=0.05, epsilon=0.05) -> None:
        ...
        self.input_shape = input_shape  # 新增，确保与map.MAP_DATA的维度匹配
        self.num_act = num_act          # 新增，确保与游戏中的动作数目匹配
    ...
    def select_action(self, obs, act):
        # 将obs转换为适合神经网络输入的形式，假设obs是从env.reset()或env.step()获得的
        obs = torch.tensor(obs.reshape(1, *self.input_shape), dtype=torch.float).to(self.device)
